...
//! (usually in a Binary expression, but see `./exprs/ops.rs` for specifics), the corresponding Id's
//! will be merged within a Union-Find structure, which is stored within an injected `ATI_ANALYSIS`
//! global. See `/src/ati/ati.rs` for a description of how the global interaction state is managed.
//!
...
//! require Id and the refered to T to be contigous in memory.
//!
...
//! ```
//!
//! `slice` should be a reference same length-id as used by `array`, but it will hold a
//! completely different set of elements inside of  the slice itself! We cannot guarantee that
//! the length-id, and the data itself will be contigous. Therefore, we convert &Tagged<T> into
//! a `TaggedRef<T> = (&Id, &T)` or a `TaggedRefMut<T> = (&mut Id, &mut T)`, by utilizing the
//! runtime library `as_tagged_ref` / `as_tagged_ref_mut` functions defined on all Tagged<T> types.
//! It's for this reason the first gather pass has to locate all places where a reference to a
//! Tagged type is constructed, so that the appropriate method call can be inserted.
//!
//! As a further compliation of the reference model, following ownership semantics, a `&mut T` does
//! not hold ownership of the underlying `T``. To disallow aliasing, it is explicitly made non-Copy.
//! It is not an "owned type". However, `TaggedRefMut<T> = (&mut Id, &mut T)` is just a tuple, and
...
//! `TaggedRefMut<T>` can be moved into functions freely. This is handled by the runtime libraries
//! `reborrow()` function, and explains why the first pass looks for expressions which evaluate to
//! a `&mut T`.
//!
//! Types, which appear above in things like ImplBlocks, but also in Stmts, FnSigs, StructDefs,
//! EnumDefs, and other AST nodes, are appropriately rewritten to convert each atomic T into a
//! Tagged<T> version (or &T to a TaggedRef<T>, etc...).
//!
...
//! 1. [crate::instrument::types] for how individual types are transformed into Tagged versions.
//! 2. [crate::instrument::expr] for how individual expressions are transformed.
//! 3. [crate::instrument::item] for how individual items are transformed.
//! 4. [crate::instrument::hoisting] for how the hoisting mechanism works.
//! 5. `/src/ati/ati.rs` for how global analysis state is managed.
//! 6. `/src/ati/index.rs` for how arrays and slices are able to be indexed with usizes and ranges.
//! 7. `/src/ati/tagged_ops.rs` for how interactions between values are observed and recorded.
//! 8. `/src/ati/tagged.rs` for Tagged/TaggedRef/TaggedRefMut definitions and further formalization.
//! 9. `/src/ati/iterators.rs` for how tagged arrays and slices are able to be used in for loops.

use crate::{
    common::DatirConfig,
    gather::first_pass_info::FirstPassInfo,
